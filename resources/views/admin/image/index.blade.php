                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        <div class="avatar">
                                            <div class="w-24 rounded">
                                                <a href="{{ url('storage/images/' . $image->path) }}" target="_blank">
                                                    <img src="{{ url('storage/images/thumbnails/' . $image->path) }}" class="w-[100px] h-[100px] object-cover rounded shadow" width="100" height="100" />
                                                </a>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                        {{ url('storage/images/' . $image->path) }}<br>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-right space-x-2">
                                        <form action="{{ route('admin.image.destroy', $image->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                            @csrf
                                            @method('DELETE')
