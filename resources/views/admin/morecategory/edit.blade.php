<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">            
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.morecategory.update', ['morecategory' => $subcategory->id]) }}">
                        @method('put')
                        @csrf
                        ...
                        <div>
                            ...
                            <select name="category_id" id="category_id" class="block w-full mt-1 px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                                ...
                                <option value="{{ $category->id }}" @selected(old('category_id', $subcategory->subcategory->category->id) == $category->id)>{{ $category->name }}</option>
                                @endforeach
                            </select>
    ...
    <script>
        ...
        const oldCat = parseInt("{{ old('category_id') ?? $subcategory->subcategory->category->id }}");
        const oldSubCat = parseInt("{{ old('subcategory_id') ?? $subcategory->subcategory_id }}");
