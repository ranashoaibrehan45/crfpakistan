<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-primary-link href="{{ route('admin.morecategory.create') }}" label="Add New" />
                            ...
                            @foreach($morecategories as $subcategory)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-right space-x-2">
                                        <a href="{{ route('admin.morecategory.edit', $subcategory->id) }}"
                                            class="inline-flex items-center px-3 py-1 text-sm text-white bg-gray-800 hover:bg-blue-700 rounded border border-black">
                                                Edit
                                            </a>
                                        <form action="{{ route('admin.morecategory.destroy', $subcategory->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                            @csrf
                                            @method('DELETE')
